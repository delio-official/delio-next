  ...
  return (
    <div style={{ background:'#fff', minHeight:'100vh' }}>
      ...
      {detailOrder && (() => {
        ...
        /* ══ PC: 넓은 2단 레이아웃 (모바일은 아래 기존 모달 유지) ══ */
        if (!isMobileView) {
            ...
            <div className="hide-scrollbar" style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', zIndex:3100, overflowY:'auto', padding:'40px 20px' }}
              ...
              <div style={{ maxWidth:920, margin:'0 auto', background:'#F7F7F5', borderRadius:16, overflow:'hidden' }} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', padding:'26px 32px 20px', background:'#fff', borderBottom:'1px solid #F0F0F0' }}>
                  <div>
                    <div style={{ fontSize:22, fontWeight:800, color:'#1A1A1A' }}>주문상세</div>
                    <div style={{ fontSize:13, color:'#999', marginTop:8 }}>{new Date(o.created_at).toLocaleDateString('ko-KR')} 주문 · 주문번호 {o.order_no}</div>
                  </div>
                  <button onClick={() => setDetailOrder(null)} style={{ background:'none', border:'none', cursor:'pointer', fontSize:22, color:'#888', lineHeight:1, padding:0 }}>✕</button>
                </div>
                <div style={{ padding:'24px 32px 32px', display:'flex', flexDirection:'column', gap:20 }}>
                    ...
                    {o.order_items?.map((item, i) => (
                      <div key={i} style={{ display:'flex', gap:14, alignItems:'center', padding:'12px 0', borderTop: i>0?'1px solid #F4F4F2':'none' }}>
                        ...
                        <div style={{ display:'flex', gap:8, flexShrink:0 }}>
                          <button style={pcBtn} onClick={() => { setAskModal({ items:[{ productId:item.product_id||'', productName:item.product_name, thumb:item.thumbnail_url??null }], selectedId:item.product_id||'' }); setAskCategory('문의'); setAskContent(''); setAskPrivate(false); }}>문의</button>
                          {(o.status==='cancelled'||o.status==='refunded') && (
                            <button style={pcBtn} onClick={() => setCancelDetail(o)}>{o.status==='cancelled'?'취소상세':'환불상세'}</button>
                          )}
                        </div>
                      </div>
                    ))}
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, alignItems:'start' }}>
                    ...
                    <section style={pcSec}>
                      <div style={pcTitle}>주문자정보</div>
                      {rowL('주문자', profile?.name || '-')}
                      {rowL('연락처', profile?.phone || o.phone || '-')}
                      {rowL('이메일', profile?.email || '-')}
                      <div style={{ marginTop:14, padding:'12px 14px', background:'#F7F7F5', borderRadius:8, fontSize:12.5, color:'#888' }}>
                        문의: 델리오 고객센터 070-8064-3601
                      </div>
                    </section>
                  </div>
                </div>
            ...
            <div className={`mp-panel${activePanel==='benefit'?' active':''}`}>
              ...
              <div className="mp-section">
                <div style={{ paddingTop:28, maxWidth:400, margin:'0 auto' }}>
                  ...
                  {/* 쿠폰 이미지 */}
                  <div style={{ maxWidth:270, margin:'0 auto 20px' }}>
                    <img src="/friend_coupon.png" alt="친구 추천 5,000원 쿠폰"
                          ...
                          <div key={q.id} style={{ borderBottom:'1px solid #F0F0F0' }}>
                            ...
                            {isOpen && (
                              <div style={{ background:'#F7F7F5', padding:'16px 0' }}>
                                ...
                                {/* 답변 */}
                                <div style={{ display:'flex', alignItems:'flex-start', ...(editQnaId === q.id ? { borderTop:'1px solid #ECECEC', paddingTop:14 } : {}) }}>
                  ...
                  <div style={{ padding:'20px 0', marginBottom:12 }}>
                    {csDone ? (
                      ...
                      <form onSubmit={submitCsInquiry}>
                        {/* 유형 선택 — 필 탭 */}
                        <div style={{ marginBottom:16 }}>
                          <div style={{ fontSize: isMobileView ? 12 : 14, fontWeight:600, color:'#aaa', marginBottom:10,
                        ...
                        <div style={{ marginBottom:12 }}>
                          <label style={{ display:'block', fontSize: isMobileView ? 12 : 14, fontWeight:600, color:'#aaa',
                          ...
                          <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'flex-start' }}>
                            ...
                            {csFiles.length < 5 && (
                              <label style={{ width:82, height:82, borderRadius:10,
                                ...
                                justifyContent:'center', cursor:'pointer', gap:4, flexShrink:0, order:-1 }}>
                          ...
                          <div key={inq.id}
                            ...
                            {isOpen && (
                              <div style={{ background:'#F7F7F5', borderRadius:10, padding:'12px 14px',
                                ...
                                {/* 문의 내용 — 수정 시에만 노출 (제목은 위 목록에 이미 표시됨) */}
                                {editCsId === inq.id && (
                                  <>
                                ...
                                {/* 답변 (대기 중이면 안내문구) */}
                                <div style={{ fontSize:11, fontWeight:700, color:'#1A1A1A', marginTop:(editCsId===inq.id || (inq.attachments && inq.attachments.length>0))?12:0, marginBottom:6 }}>💬 답변</div>
                                <p style={{ margin:0, whiteSpace:'pre-wrap', color: inq.answer ? '#555' : '#aaa' }}>{inq.answer || '아직 답변이 등록되지 않았습니다.'}</p>
