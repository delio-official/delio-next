...
export default function BrandDirectClient() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!farmData || farmData.length === 0) { setLoading(false); return; }
      ...
      (prodData as Product[] || []).forEach(p => { if (!map[p.farm_id]) map[p.farm_id] = p; });
      setProdMap(map);
      setLoading(false);
    }
    load();
  ...
  return (
    <main style={{ background: '#fff', minHeight: '100vh', paddingBottom: 80 }}>
      ...
      <div style={{
        ...
        padding: '48px 0 36px', borderBottom: '1px solid #C8DCF8',
      }}>
        <div className="container">
          <p style={{ fontSize: 11, color: '#3A6AB0', fontWeight: 700, letterSpacing: 2, marginBottom: 8 }}>
            PARTNER FARM · BRAND DIRECT
          </p>
          <h1 style={{ fontSize: 'clamp(24px,4vw,34px)', fontWeight: 800, marginBottom: 10 }}>
            브랜드 직송관
          </h1>
          <p style={{ fontSize: 14, color: '#444', lineHeight: 1.7 }}>
            믿을 수 있는 농가에서 직접 보냅니다.<br />
            산지에서 내 식탁까지 빠르게, 신선하게.
          </p>
          <div style={{ fontSize: 12, color: '#aaa', marginTop: 16 }}>
            ...
            <span style={{ margin: '0 6px' }}>/</span>
            <span style={{ color: '#555' }}>브랜드 직송관</span>
          </div>
        </div>
      </div>
      ...
      <section className="brand-direct-section" style={{ paddingTop: 40 }}>
        <div className="container">
          ...
          {loading ? (
            ...
            <div className="brand-direct-grid">
              {farms.map(farm => {
                ...
                const banner = BANNER_MAP[type] || 'bdc-banner-citrus';
                const logo = LOGO_MAP[type] || 'bdc-logo-citrus';
                const thumb = THUMB_MAP[type] || 'bdc-thumb-citrus';
                    ...
                    <div className="bdc-body">
                      ...
                      {farm.region && (
                        <div style={{ marginBottom: 12 }}>
                          <span style={{ fontSize: 11, fontWeight: 700, background: '#EEF5FF',
                            color: '#3A6AB0', padding: '2px 8px', borderRadius: 20 }}>
                            📍 {farm.region}
                          </span>
                        </div>
                      )}
          ...
          <div style={{ marginTop: 56, borderRadius: 20, padding: '40px 32px', textAlign: 'center',
            ...
            <h2 style={{ fontSize: 18, fontWeight: 800, marginBottom: 8 }}>산지 직송 파트너 모집</h2>
            <p style={{ fontSize: 13, color: '#555', lineHeight: 1.7, marginBottom: 22 }}>
              좋은 농산물을 키우고 계신가요?<br />
