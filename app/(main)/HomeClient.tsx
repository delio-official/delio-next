      ...
      <div key={p.id} className="qg-card" onClick={() => router.push(`/product/${p.id}`)}>
        ...
        <div className="qg-card-body">
          ...
          <div className="qg-card-price-row">
            ...
            {p.avg_rating > 0 && (
              <span className="qg-price-rating" onClick={e => { e.stopPropagation(); router.push(`/product/${p.id}?tab=review`); }}>
                <svg viewBox="0 0 24 24" width="14" height="14" fill="#FFB400" stroke="none" aria-hidden="true"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>
                {p.avg_rating.toFixed(1)}({p.review_count.toLocaleString()})
              </span>
            )}
          </div>
          {p.avg_rating > 0 && (
...
export default function HomeClient() {
  ...
  const [reviews, setReviews] = useState<{ id: string; image: string; images: string[]; stars: number; text: string; prodId: string; prodName: string; prodRating: string; emoji: string; prodThumb: string | null; authorName: string; sellerReply: string | null }[]>([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      type Row = { id: string; rating: number; content: string; image_urls: string[] | null; author_name: string | null; seller_reply?: string | null; products: { id: string; name: string; category: string; avg_rating: number | null; review_count: number | null; thumbnail_url: string | null } | null };
  ...
  useEffect(() => {
    (async () => {
      ...
      let farmsData: FRow[] = [];
      if (cfg.mode === 'manual' && cfg.ids.length > 0) {
        const { data } = await supabase.from('farms').select(fcols).eq('is_own', false).is('deleted_at', null).in('id', cfg.ids);
        farmsData = orderByIds((data as FRow[]) || [], cfg.ids);
      } else if (cfg.mode === 'popular') {
        const { data } = await supabase.from('farms').select(fcols).eq('is_own', false).is('deleted_at', null);
        ...
        const { data } = await supabase.from('farms').select(fcols).eq('is_own', false).is('deleted_at', null).order(ord.col, { ascending: ord.asc });
  ...
  return (
    <>
        ...
        <div className="container">
          <div className="g-section-head">
            <h2 className="g-section-title">
              ...
              <div className="g-title-main">
                <Link href="/farms" style={{ textDecoration:'none', color:'inherit' }}>브랜드 직송관</Link>
      ...
      {reviewModal && (() => {
        ...
        return (
          <ReviewPhotoModal
            review={{ id: reviewModal.id, images: reviewModal.images.length ? reviewModal.images : [reviewModal.image], rating: reviewModal.stars, content: reviewModal.text, sellerReply: reviewModal.sellerReply, authorName: reviewModal.authorName }}
