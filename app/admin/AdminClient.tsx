    ...
    <div className="adm-content">
      ...
      {smsTab === 'compose' && (
        <div className="adm-card adm-card-lg">
          ...
          <div className="adm-form" style={{ flex:'1 1 420px', minWidth:0 }}>
...
export default function AdminClient() {
  ...
  const [farmPageSize, setFarmPageSize] = useState(20);
 ...
 badge: '', badge_color: BADGE_DEFAULT_COLOR, is_new: false,
  ...
  async function downloadFarmSettlePdf(row: { farmName: string; qty: number; payout: number; orderCount: number; orders: FarmSettleOrder[] }) {
    ...
    const inner = `
      ...
      #pdfDoc h1{font-size:20px;margin:0 0 4px;} #pdfDoc h2{font-size:15px;margin:18px 0 8px;}
      #pdfDoc .sub{color:#666;font-size:13px;margin-bottom:20px;}
      #pdfDoc .box{border:1px solid #ddd;border-radius:8px;padding:14px 16px;margin-bottom:16px;}
      #pdfDoc .row{display:flex;justify-content:space-between;padding:4px 0;font-size:14px;}
      #pdfDoc table{width:100%;border-collapse:collapse;font-size:13px;margin-bottom:8px;}
      #pdfDoc th,#pdfDoc td{border:1px solid #e0e0e0;padding:8px 10px;}
      #pdfDoc th{background:#f5f5f5;text-align:left;} #pdfDoc .total{text-align:right;font-size:16px;font-weight:800;margin-top:12px;}
      </style>
      <div id="pdfDoc">
        <h1>정산 명세서</h1>
    ...
    try {
      ...
      const target = container.querySelector('#pdfDoc') as HTMLElement;
      ...
      pdf.save(`정산명세서_${row.farmName}_${farmSettleMonth}_${farmSettleHalf}차.pdf`);
    } catch (e) {
  ...
  return (
    <div style={{ background:'#F1F5F9', minHeight:'100vh', fontFamily:"'Pretendard', -apple-system, sans-serif" }}>
        ...
        <div className="adm-productpage">
          <div className="adm-productpage-inner">
            ...
            <div className="adm-modal-body" style={{ display:'flex', flexDirection:'column', gap:14 }}>
                        ...
                        {(() => {
                          ...
                          return (
                            <div style={{ marginTop:12, paddingTop:12, borderTop:'1px solid #E2E8F0', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                              ...
                              <span>
                                <b style={{ fontSize:18, color:'#1A1A1A' }}>{fmtPrice(sell)}</b>
                                <span style={{ fontSize:13, color:'#1A1A1A' }}>원</span>
              ...
              <div className="adm-flex-gap adm-flex-end" style={{ marginTop:4, alignItems:'center' }}>
                {editingProduct && (
                  <button className="adm-btn adm-btn-outline" style={{ marginRight:'auto', color:'#DC2626', borderColor:'#FECACA' }} disabled={pSaving}
                    onClick={async () => { const ok = await deleteProduct(editingProduct); if (ok) closeProductForm(); }}>
                    상품 삭제
                  </button>
                )}
      ...
      <div className="adm-wrap">
        ...
        <div className="adm-main">
          ...
          {panel === 'dashboard' && (
            <div className="adm-content">
              ...
              <div className="adm-card" style={{ marginBottom:24 }}>
                ...
                <div className="adm-kpi-grid adm-kpi-5 adm-kpi-grid-flat">
                {statsLoading ? (
              ...
              <div className="adm-toolbar">
                ...
                <div className="adm-toolbar-right">
                  ...
                  <button className="adm-btn adm-btn-outline" onClick={() => {
                    ...
                    }} title="델리오(자사) 상품 주문서 — CJ대한통운 대량접수 업로드용 양식">
                    <span className="adm-btn-icon">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                    </span>
                    자사상품 주문서
                    ...
                    <table className="adm-table adm-table-center" style={{ tableLayout:'fixed' }}>
                      <thead>
                        <tr>
                          ...
                          <th style={{ width:'10%' }}>주문번호</th><th style={{ width:'12%' }}>주문일시</th><th style={{ width:'7%' }}>주문자</th>
                          <th style={{ width:'18%' }}>상품</th>
                          <th style={{ width:'13%' }}>금액</th><th style={{ width:'9%' }}>상태</th><th style={{ width:'20%' }}>송장번호</th><th style={{ width:'9%' }}>관리</th>
                        </tr>
                      </thead>
                      <tbody>
                        {filteredOrders.length === 0 ? (
                          ...
                          return groups.map((g, gi) => {
                            ...
                            return (
                            /* 같은 주문의 브랜드 이어지는 줄만 연한 점선. 주문 사이는 표 기본선 그대로 */
          ...
          {panel === 'products' && (
            <div className="adm-content">
              ...
              <div className="adm-toolbar">
                ...
                <div className="adm-toolbar-right">
                  {(productSearch || productCatFilter || productBrandFilter || productStatusFilter || productTypeFilter) && (
                    <button className="adm-btn adm-btn-outline" onClick={() => { setProductSearch(''); setProductCatFilter(''); setProductBrandFilter(''); setProductStatusFilter(''); setProductTypeFilter(''); setProductPage(1); }}>초기화</button>
                  )}
                  ...
                  <>
                    ...
                    {navItems.map(m => (
                      <div key={m.id} className="adm-card" style={{ padding:'10px 14px', marginBottom:8, display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}
                        ...
                        <span style={{ display:'inline-flex', alignItems:'center', gap:6, fontSize:11, color:'#475569', flexShrink:0 }}>상단바 노출 <AdmToggle on={m.show_in_header} color="#2563EB" onChange={v => updateMenu(m.id, { show_in_header: v })} title="상단바 노출" /></span>
          ...
          {panel === 'farms' && (() => {
            ...
            const farmTotalPages = Math.max(1, Math.ceil(filteredFarms.length / farmPageSize));
            const farmCurPage = Math.min(farmPage, farmTotalPages);
            const pagedFarms = filteredFarms.slice((farmCurPage - 1) * farmPageSize, farmCurPage * farmPageSize);
            return (
            <div className="adm-content">
              <div className="adm-toolbar" style={{ flexWrap:'wrap', gap:8 }}>
                <div className="adm-toolbar-left">
                ...
                <div className="adm-toolbar-right">
                  <AdmSelect value={String(farmPageSize)} onChange={v => { setFarmPageSize(Number(v)); setFarmPage(1); }}
                    options={[10,20,30,50,100].map(n => ({ value:String(n), label:`${n}개씩` }))} />
                  {farmSearch && (
                    <button className="adm-btn adm-btn-outline" onClick={() => { setFarmSearch(''); }}>초기화</button>
                  )}
            ...
            <div className="adm-content">
              ...
              <div className="adm-toolbar" style={{ flexWrap:'wrap', gap:8 }}>
                ...
                <div className="adm-toolbar-right" style={{ gap:8 }}>
                  ...
                  {(reviewRating || reviewFarm || reviewFlag || reviewAnswered !== 'all' || reviewSearch || reviewFrom || reviewTo) && (
                    <button className="adm-btn adm-btn-outline" onClick={() => { setReviewRating(''); setReviewFarm(''); setReviewFlag(''); setReviewAnswered('all'); setReviewSearch(''); setReviewFrom(''); setReviewTo(''); setReviewPage(1); loadReviews(); }}>초기화</button>
                  )}
              ...
              {couponTab === 'tab-coupon' ? (
                <>
                  ...
                  <div className="adm-toolbar" style={{ marginTop:0, flexWrap:'wrap', gap:8 }}>
                    ...
                    <div className="adm-toolbar-right" style={{ flexWrap:'wrap', gap:8, alignItems:'center' }}>
                      {(clStatus !== 'all' || clCategory !== 'all' || clSearch) && (
                        <button className="adm-btn adm-btn-outline" onClick={() => { const d = new Date(); d.setMonth(d.getMonth()-1); setClStatus('all'); setClCategory('all'); setClSearch(''); setClFrom(ymd(d)); setClTo(ymd(new Date())); setClPage(1); loadCouponLogs(); }}>초기화</button>
                      )}
                ...
                <>
                  ...
                  <div className="adm-card" style={{ marginBottom:24, padding:'20px 22px' }}>
                    <div className="adm-card-head" style={{ marginBottom:16, borderBottom:'none', alignItems:'flex-start' }}>
                      <div>
                        <span className="adm-card-title">멤버십 등급 설정</span>
                  ...
                  <div className="adm-toolbar">
                    ...
                    <div className="adm-toolbar-right">
                      {(pointSearch || pointFilter !== 'all') && (
                        <button className="adm-btn adm-btn-outline" onClick={() => { setPointSearch(''); setPointFilter('all'); }}>초기화</button>
                      )}
          ...
          {panel === 'events' && (
            <div className="adm-content">
              <div className="adm-toolbar" style={{ flexWrap:'wrap', gap:8 }}>
                ...
                <div className="adm-toolbar-right" style={{ gap:8 }}>
                  {(evStatusFilter !== 'all' || evSearch || evFrom || evTo) && (
          ...
          {panel === 'lounge' && (
            <div className="adm-content">
              ...
              <div className="adm-toolbar" style={{ flexWrap:'wrap', gap:8 }}>
                ...
                <div className="adm-toolbar-right" style={{ gap:8 }}>
                  {(loungeFilter || loungeStatusFilter !== 'all' || loungeSearch) && (
                    <button className="adm-btn adm-btn-outline" onClick={() => { setLoungeFilter(''); setLoungeStatusFilter('all'); setLoungeSearch(''); }}>초기화</button>
                  )}
            ...
            <div className="adm-content">
              ...
              <div className="adm-toolbar">
                ...
                <div className="adm-toolbar-right">
                  ...
                  {(memberSearch || memberGradeFilter || memberBlockFilter !== 'all' || memberProviderFilter || memberMktFilter !== 'all' || memberNewOnly) && (
                    <button className="adm-btn adm-btn-outline" onClick={() => { setMemberSearch(''); setMemberGradeFilter(''); setMemberBlockFilter('all'); setMemberProviderFilter(''); setMemberMktFilter('all'); setMemberNewOnly(false); setMemPage(1); }}>초기화</button>
                  )}
                        ...
                        return (
                          <div style={{ flex:'3 1 200px', minWidth:160, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:4, borderLeft:'1px solid #F1F5F9', paddingLeft:16 }}>
                            ...
                            <svg width={128} height={128} viewBox="0 0 120 120">
                              ...
                              <text x="60" y="75" textAnchor="middle" fontSize="10" fill="#94A3B8">마진율</text>
                            </svg>
                            <span style={{ fontSize:14, fontWeight:800, color:'#16A34A' }}>{fmtPrice(settlementData.margin)}원</span>
                            <span style={{ fontSize:10, color:'#94A3B8' }}>상품원가 {fmtPrice(settlementData.supplyCost)}원</span>
                          </div>
                        );
                      })()}
                    </div>
              ...
              <div className="adm-content">
                ...
                {typeSorted.length > 0 && (
                  <div className="adm-card" style={{ marginBottom:16 }}>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(150px,1fr))', gap:10, padding:'14px 18px' }}>
                      {/* 전체 */}
              ...
              <div className="adm-card" style={{ marginBottom:20 }}>
                ...
                <div style={{ display:'flex', gap:24, flexWrap:'wrap', alignItems:'flex-start' }}>
                  ...
                  <div style={{ flex:'7 1 420px', minWidth:0, display:'flex', flexDirection:'column', gap:20 }}>
                    ...
                    <div>
                      <div className="adm-muted" style={{ fontSize:12, marginBottom:8, fontWeight:600 }}>푸터 미리보기 · 모바일</div>
                      {renderFooter(true)}
                    </div>
                    <div className="adm-muted" style={{ fontSize:11 }}>* 운영시간·입금계좌 등은 코드 기본값입니다(현재 편집 대상 아님).</div>
                  </div>
                </div>
              </div>
                );
          ...
          <div className="adm-modal adm-modal-wide" onClick={e => e.stopPropagation()}>
            ...
            <div className="adm-modal-body">
              <div className="adm-detail-2col">
                ...
                {/* ── 우: 맛 프로파일 · 신고 · 답변 ── */}
                <div className="adm-detail-col" style={{ display:'flex', flexDirection:'column', gap:12 }}>
                  {/* 맛 프로파일 — 구매자 행 / 판매자 행으로 나눠 비교 */}
                  {selectedReview.taste && Object.keys(selectedReview.taste).length > 0 && (() => {
                    const sscore = selectedReview.products?.seller_score || null;
                    const gridSt: React.CSSProperties = { display:'grid', gridTemplateColumns:'repeat(5, 1fr)', gap:5 };
                    const cellSt: React.CSSProperties = { background:'#F8FAFC', border:'1px solid #F0F0EE', borderRadius:8, padding:'7px 4px', textAlign:'center' };
                    const axisSt: React.CSSProperties = { fontSize:10, color:'#94A3B8', fontWeight:600 };
                    const roleSt: React.CSSProperties = { fontSize:11, fontWeight:700, color:'#334155', margin:'2px 0 4px' };
                    return (
                    <div style={{ textAlign:'left' }}>
                      <div style={{ fontSize:12, fontWeight:700, color:'#475569', marginBottom:6 }}>
                        맛 프로파일 <span style={{ fontWeight:400, color:'#94A3B8' }}>(구매자 응답 vs 판매자 설정)</span>
                      </div>
                      {/* 구매자 응답 행 */}
                      <div style={roleSt}>구매자</div>
                      <div style={gridSt}>
                        {TASTE_AXES.map(axis => {
                          const bLv = selectedReview.taste?.[axis.key];
                          const sRaw = axis.sellerSet ? sscore?.[axis.key] : undefined;
                          const sLv = sRaw != null ? toLevel(sRaw) : null;
                          const match = sLv != null && bLv != null ? Math.abs(sLv - bLv) <= 1 : null;
                          return (
                            <div key={axis.key} style={cellSt}>
                              <div style={axisSt}>{axis.label}{match === false && <span style={{ color:'#DC2626', marginLeft:2 }}>≠</span>}{match === true && <span style={{ color:'#16A34A', marginLeft:2 }}>✓</span>}</div>
                              <div style={{ fontSize:11, fontWeight:800, lineHeight:1.25, marginTop:4, color: bLv != null ? axis.hex : '#CBD5E1', wordBreak:'keep-all' }}>
                      ...
                      {/* 판매자 설정 행 */}
                      <div style={{ ...roleSt, marginTop:8 }}>판매자</div>
                      <div style={gridSt}>
                        {TASTE_AXES.map(axis => {
                          const sRaw = axis.sellerSet ? sscore?.[axis.key] : undefined;
                          const sLv = sRaw != null ? toLevel(sRaw) : null;
                          return (
                            <div key={axis.key} style={cellSt}>
                              <div style={axisSt}>{axis.label}</div>
                              <div style={{ fontSize:11, fontWeight:600, lineHeight:1.25, marginTop:4, color: sLv != null ? '#64748B' : '#CBD5E1', wordBreak:'keep-all' }}>
      ...
      {/* ===== 쿠폰 생성/수정 모달 ===== */}
      {couponModal && (
        <div className="adm-modal-bg open" onClick={() => setCouponModal(false)}>
          {/* 다른 상세와 같은 규격. 2단 배치로 스크롤 없이 한 화면에 */}
              ...
              <div className="adm-detail-grid adm-form-compact">
                ...
                <div className="adm-form-row adm-form-row-full">
                  ...
                  <div className="adm-flex-center-gap">
                    ...
                    <span className="adm-muted">일</span>
                    <span style={{ fontSize:11, color: couponForm.signup_grant ? '#B45309' : '#94A3B8', lineHeight:1.4 }}>
                      {couponForm.signup_grant
                        ? <>신규회원 쿠폰은 <strong>가입일 + N일</strong>로 만료됩니다. 반드시 일수를 입력하세요.</>
                        : <>비우면 위 고정 만료일을 씁니다. 입력하면 <strong>발급일 + N일</strong>.</>}
                    </span>
                  </div>
                </div>
              </div>
