...
export async function getDownloadableCoupons(userId: string): Promise<PublicCoupon[]> {
  ...
  const { data: pub } = await supabase
    .from('coupons')
    .select('id, name, discount_type, discount_value, min_order_amount, max_discount_amount, starts_at, expires_at')
    .eq('is_public', true)
    .eq('is_active', true);
...
export async function claimAllPublic(userId: string): Promise<number> {
  ...
  const supabase = createClient();
  const rows = list.map((c) => ({ user_id: userId, coupon_id: c.id, expires_at: c.expires_at }));
